 ...
 * 下面的配置就不一一翻译了，从Region1到Region255代表了RegionID的
 * 256个图块
 * 默认值default为0 代表该regionID没有绑定任何公共事件，
 * 如果你想绑定公共事件就把0改为公共事件的编号
 * 举例：我定义了一个公共事件0005,事件触发效果为屏幕闪烁、
 * 玩家队伍HP减少10%
 * 我现在把region1的default值改为5，
 * 并在地图上所有沼泽区域覆盖region1图层
 * 那么当角色进入所有沼泽地形时就会触发上述事件，实现地形伤害效果。
 * 这种方法比修改伤害地形类型有效，在于事件可以触发更多连锁效果，
 * 比如闪屏、弹出对话框等给特殊区域绑定公共事件。
 * 走到贴上公共事件的区域就会触发特殊事件
 * 
 * 警告：如果你设置了遭遇模式，这不会降低步行遭遇数。
 * 这是MV软件检索的。
 * 类似如果你放置了触碰事件，步行遭遇数也不会降低。
 * 如果你在事件触发时改变，可以用上诉脚本命令
 * 
 * 地图上有255个区域可以标记。
 * 你可以通过插件绑定事件ID，来让玩家步行此区域触发。
 ...
 * Map Notetags:
 *   <Region x Event: y>
 *   自定义region ID为X的图层块，会触发公共事件Y。
 *   注意X范围为1-255，Y的范围为你自定义公共事件的编号。
 *
 * ============================================================================
 * Changelog
 * ============================================================================
